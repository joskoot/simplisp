...
If the @nbpr{promise-state} is @nbr['delay]
...
If the @nbpr{promise-state} @nbr['forced] the content is returned.

If the @nbpr{promise-state} is @nbr['error]
the exception memorized in the content is raised again.
...
After all @nbr[expr]s have been evaluated and assigned,
the @nbr[body-expr]s are evaluated. Allows recursion:
...
@(hspace 4)(@nbpr{quote}@(lb)
@(hspace 5)(@nbpr{let} ((@tt{unquote-splicing} 4) (@tt{unquote} @nbr[add1]))@(lb)
@(hspace 6)(@nbpr{quasiquote} (a b unquote (@tt{unquote-splicing} (@nbr[map] unquote '(0 1 2 3)))@(lb)
@(hspace 7)(unquote (unquote unquote-splicing)))))))}@(lb)
