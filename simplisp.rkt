...
;
; However, including quoted source-code only inhibits DrRacket to show binding information and hyper-
; links to documentation when background expansion is enabled or the Check Syntax button is selected.
; Notice that simplisp has a different procedure |eval| than racket/base:
;
;   (simplisp `(eval ',source-code)) or
;   (simplisp `(eval ',source-code base-env))

(define-syntax-rule
  (define-quoted-and-evaluated (simplisp source-code)         expr        )
  (define-values               (simplisp source-code) (values expr 'expr)))

;=====================================================================================================
; The source-code consiste of one single expression.

(define-quoted-and-evaluated (simplisp source-code)
    ...
    (((register) '())
     ...
     (($special-vars)
      (register-put! 'special-vars 'proc ; A thunk, because we must wait until fill-clean-environment!
        (λ () special-var-names)))       ; has assigned the list of special-var-names.
     ...
     (($trace-width)
      (register-put! 'trace-width #f
        (make-parameter default-width trace-width-guard 'trace-width)))
     ...
     ;-----------------------------------------------------------------------------------------------
     ; A macro is a struct with procedure property.
     ; Made for predefined syntactic forms and those made by macro macro.
        ...
        (((printer)
         ...
         ((descr constr pred acc mut)
          (make-struct-type
            ...
            #f)))    ; no guard
        (values
          (register-put! 'base-env  #f (constr 'base-env (hasheq)))
          (register-put! 'base-env? #f pred))))
     ...
     ;-----------------------------------------------------------------------------------------------
     ; Undefined, constant and predicate.
     ...
     (($make-promise-type) ; --> (values make-promise promise? delay lazy force)
     ...
     ;-----------------------------------------------------------------------------------------------
     ; Stream-types.
     ; Procedure |$make-stream-type| returns multiple value:
     ;    stream-lazy
     ;    stream-cons
     ;    stream
     ;    stream-car
            ...
            (cond
              ...
              (else
                (let loop ((expr (car exprs)) (exprs (cdr exprs)))
                  (set! trace-id 0)
                  (cond
                    ((null? exprs) (*eval expr $base-env))
                    (else (*eval expr $base-env) (loop (car exprs) (cdr exprs))))))))
          'simplisp)))
     ...
     ((evalargs) ; Treat elements of multiple values as separate arguments.
      (λ (uargs env)
     ...
     ; Procedure |eval-to-list| used in binding forms with multiple values..
     ...
     ;-----------------------------------------------------------------------------------------------
     ; Predefined macros and procedures.
     ...
     ; Macro |macro|
     ; A macro is like a procedure, but is called with unevaluated arguments.
     ; The body must yiled one value, which is evaluated in the environment
          ...
          (λ (uargs env)
            ...
            (let ((id (*eval (car uargs) env)))
              ...
              (append
                ...
                (if
                  (eq? (namespace-variable-value id #t (λ () not-found) clean-environment)
                  not-found)
                  '() '(clean))
                (if
                  (hash-has-key? (global-table-hash ($current-global-table)) id)
                  '(global)
                  '())))))))
        ...
        (λ (uargs env)
          ...
          (let*
            ((boundlist (car uargs))
             ...
             (predicate/handlers
               (map (λ (binding) (cons (*eval (car binding) env) (*eval (cadr binding) env)))
                 boundlist)))
     ...
     (($eval) ; Use rest-argument for optional env.
      (register-put! 'eval 'proc
        (λ (expr . wrapped-env) ; rest argument for optional argument env.
          (check-eval-env-arg (cons expr wrapped-env))
          (let*
            ((env (if (null? wrapped-env) $base-env (car wrapped-env))))
            (*eval expr env)))))
     ...
     ;-----------------------------------------------------------------------------------------------
     ; Helper procedures. To do: complete procedures that check arguments of macros and procedures.
     ...
     ((eval-args?) (λ (x) (or (= (length x) 1) (and (= (length x) 2) ($env? (cadr x))))))
     ...
     ((check-macro)         (make-check 'macro lambda-uargs?))
     ...
     ((check-cond)          (make-check 'cond (λ (x) (and (andmap list? x) (andmap pair? x)))))
     ...
     ((check-eval-env-arg)  (make-check 'eval eval-args?))
